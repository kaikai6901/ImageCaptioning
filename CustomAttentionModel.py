...
class CustomAttentionModel():
    ...
    def beam_search_predict(self, image, kbeams=3):
        attention_plot = np.zeros((max_length, self.attention_feature_shape))
        ...
        features = self.encoder(img_tensor_val) 
        # dec_input = tf.expand_dims([self.word_to_index['start']], 0)
        start_word = [[[self.word_to_index['start']], 0.0, hidden]]

        while(len(start_word[0][0]) < max_length):
            temp = []
            for s in start_word:
                predictions, hidden, attention_weights = self.decoder(tf.expand_dims([s[0][-1]], 0), features, s[2]) 
                word_preds = np.argsort(predictions[0])[-kbeams:]
                for w in word_preds:
            ...
            start_word = start_word[-kbeams:]
        start_word = start_word[-1][0]
        captions_ = [self.index_to_word[i] for i in start_word]
        final_caption = []
        for i in captions_:
            if i != 'end':
    ...
    def generate(self,image_path):
        ...
        cap_with_beam = self.beam_search_predict(image_path)
